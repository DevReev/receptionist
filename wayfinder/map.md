...
- Domain: Node webhook server (always-on host, public URL; laptop tunnels dev-only) driving an upgraded Twilio voice number, turn-based (`<Record>` → whisper-1 → LLM → `<Say>`). No streaming/barge-in.
- Locked by the human: whisper-large STT only; Twilio `<Say>` fixed voice/language; single hand-edited `clinic.md`, hot-reloaded by file-watch; info + appointments only, medical advice refused with handoff line; slots only from Picktime; failures to console log (no SMS).
...
- Streaming voice / barge-in and neural/expressive TTS voices (v1 is turn-based `<Say>`).
