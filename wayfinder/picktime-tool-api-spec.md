...
- Slot mapping: page-local slot ints stay internal; outside the service every time is ISO local with the configured timezone end to end; Availability responses carry fetch time and explicit none-available reasons. `locationId` is optional for search and required for Booking. Canonical date query names are `from` and `to`; `startDate` and `endDate` are compatibility aliases for tool platforms that reserve those names.
