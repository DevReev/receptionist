...
- Second half (same call): with windows flowing, real-speech VAD scores flicker across the 0.5 threshold (max consecutive run 160 ms vs the 300 ms strict-consecutive latch), so `speaking` still never latched — proven by simulated-caller injection over public `wss` (utterance never fired, 1 outbound frame). Fix: pre-latch dip tolerance (`latchDipMs`, default 200 ms, `ENDPOINT_LATCH_DIP_MS`) — brief flicker keeps gathering, longer dips reset. The 200 ms-noise rejection test still passes. New flicker tests in `test/endpoint.test.ts`. Same simulation then completed two full Turns live (endpoint → Groq STT → DeepSeek reply → OpenRouter TTS → spoken reply, Turn logs for both). Full suite 91/91 green.
